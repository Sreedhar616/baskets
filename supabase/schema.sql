...
create table if not exists public.products (
  ...
  slug          text not null unique,
  description   text,
  price         integer not null check (price >= 0),       -- ONLINE price, paise
  cod_price     integer not null default 0 check (cod_price >= 0), -- COD price, paise
  compare_price integer check (compare_price >= 0),         -- optional MRP
  images        text[] not null default '{}',
  sizes         jsonb not null default '[]',                -- [{label,online,cod}] paise; [] = no sizes
...
create trigger trg_products_updated before update on public.products
  for each row execute function public.set_updated_at();
-- Adds columns to databases created before these existed (safe to re-run).
alter table public.products add column if not exists sizes jsonb not null default '[]';
alter table public.products add column if not exists cod_price integer not null default 0 check (cod_price >= 0);
